...
void munkki::draw(float time)
{
    ...
    glTranslatef(0, (movement-movement2)*10, -11);
    ...
    const float wobblepower = calcPosFloat(framePosition, 0.01f, 0.25f);
    float radius1 = 3.0f;
    //ulompi kehä
    for (y=0;y<yres;y++)
    {
        ...
        float radius2 = 1.0f + wobblepower * 0.3f*(float)cos(ykohta*4*3.141592f+framePosition*55);
        ...
        for (x=0;x<xres;x++)
        {
            ...
            Vector edgepoint = Vector(0, (float)sin(xangle), (float)cos(xangle))*radius2
                *(float)(1.0f * (1-wobblepower) + wobblepower * (1.0f+0.1f*cos(xkohta*7*3.141592f+framePosition*35))) ;
